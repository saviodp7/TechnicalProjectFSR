...
class Traj_planning:
    ...
    def cartesian_traj(self,fs = 10,profile_type=LINEAR_TYPE):
        path_length = 0
        optimal_path = []
        for i in range(len(self.path_list) - 1):
            x_path,y_path,theta_path,v,w = self.cartesian_poly(self.path_list[i], self.path_list[i + 1],fs=fs,profile_type=profile_type)
            combined = list(zip(x_path, y_path,theta_path))
            optimal_path.extend(combined)
