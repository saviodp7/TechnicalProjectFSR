...
class PostureRegulation:
    ...
    def trigger_next_timer(self):
        self.current_point_index += 1
        if self.current_point_index < len(self.trajectory_points):
            next_timer = self.trajectory_timers[self.current_point_index]
            next_point = self.trajectory_points[self.current_point_index]
            next_timer.init(mode=Timer.ONE_SHOT, period=int(self.dt * 1000), callback=lambda t: self.go(next_point))
        else:
